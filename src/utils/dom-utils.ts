...
export class DOMUtils {
  static isValidBlock(block: Element): boolean {
    const hasElClass = Array.from(block.classList).some((cls) => cls.startsWith("el-"));
  ...
  static getValidBlocks(): HTMLElement[] {
    return Array.from(document.querySelectorAll(DOM_SELECTORS.MARKDOWN_SECTION)).filter((block) => this.isValidBlock(block)) as HTMLElement[];
  }
  ...
  static scrollToBlock(block: HTMLElement): void {
    ...
    const scrollTop = blockRect.top - containerRect.top - (containerRect.height - blockRect.height) / 2;
  ...
  static clearHighlights(): void {
    document.querySelectorAll(`.${READER_CLASSES.highlight}`).forEach((el) => el.classList.remove(READER_CLASSES.highlight));
  ...
  static getCurrentBlockIndex(): number {
    ...
    const currentBlock = document.querySelector(`.${READER_CLASSES.highlight}`) as HTMLElement;
    return currentBlock ? blocks.indexOf(currentBlock) : -1;
  }

  static isDOMReady(): boolean {
    return !!this.getMarkdownPreviewView() && this.getValidBlocks().length > 0;
  }
