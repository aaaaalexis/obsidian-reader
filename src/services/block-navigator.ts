...
export class BlockNavigator {
  ...
  async navigateToBlock(direction: NavigationDirection): Promise<void> {
    ...
    const nextIndex = currentIndex === -1 ? 0 : direction === "previous" ? Math.max(0, currentIndex - 1) : Math.min(blocks.length - 1, currentIndex + 1);
  ...
  async restorePosition(): Promise<void> {
    ...
    const position = await ProgressStorage.loadPosition(this.plugin, activeFile.path);

    if (!position) {
      await ProgressStorage.savePosition(this.plugin, activeFile.path, -1, false);
      this.enabled = false;
  ...
  private async savePosition(): Promise<void> {
    ...
    const currentIndex = DOMUtils.getCurrentBlockIndex();
    if (currentIndex >= 0) {
      await ProgressStorage.savePosition(this.plugin, activeFile.path, currentIndex);
    }
  }
  ...
  removeMobileUI(): void {
    const buttons = document.querySelector(`.${READER_CLASSES.blockButtons}`);
    buttons?.remove();
  }

  private createNavigationButton(direction: NavigationDirection | "scroll", icon: string): HTMLElement {
    ...
    button.addEventListener("click", async (evt) => {
      evt.stopPropagation();
      if (direction === "scroll") {
        const highlightedBlock = document.querySelector(`.${READER_CLASSES.highlight}`);
        if (highlightedBlock) {
